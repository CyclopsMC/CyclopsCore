...
/**
 * The action used for {@link FluidConfigNeoForge}.
 * @author rubensworks
...
public class FluidActionNeoForge extends ConfigurableTypeActionCommon<FluidConfigNeoForge, BaseFlowingFluid.Properties, ModBaseNeoForge<?>> {
    ...
    private final Multimap<String, Pair<FluidConfigNeoForge, Callable<?>>> registryEntriesHolder = Multimaps.newListMultimap(Maps.<String, Collection<Pair<FluidConfigNeoForge, Callable<?>>>>newHashMap(), new com.google.common.base.Supplier<List<Pair<FluidConfigNeoForge, Callable<?>>>>() {
        ...
        @Override
        public List<Pair<FluidConfigNeoForge, Callable<?>>> get() {
    ...
    public static void onRegistryEvent(RegisterEvent event) {
        ((FluidActionNeoForge) ConfigurableTypesNeoForge.FLUID.getConfigurableTypeAction()).onRegistryEventInner(event);
    }
    ...
    public void onRegistryEventInner(RegisterEvent event) {
        if (event.getRegistryKey() == BuiltInRegistries.FLUID.key()) {
            ...
            registryEntriesHolder.get(registry.key().toString()).forEach((pair) -> {
                FluidConfigNeoForge config = pair.getLeft();
            ...
            registryEntriesHolder.get(BuiltInRegistries.FLUID.key().toString()).forEach((pair) -> {
                ...
                event.register((ResourceKey) event.getRegistry().key(), Identifier.fromNamespaceAndPath(config.getMod().getModId(), config.getNamedId()), fluidType);
            });
        }
    }

    protected void registerFluid(Registry<Fluid> registry, RegisterEvent event, Supplier<Fluid> fluidSupplier, Identifier name) {
